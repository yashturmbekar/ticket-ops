...
export async function updateAsset(id: string, assetData: Record<string, unknown>) {
...
export async function uploadAssetAttachment(assetId: string, file: File) {
  ...
  return apiClient.post(`${endpoint}/${assetId}/attachments`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
}

export async function deleteAssetAttachment(assetId: string, attachmentId: string) {
...
export async function bulkUpdateAssets(assetIds: string[], updates: Record<string, unknown>) {
  return apiClient.patch(`${endpoint}/bulk-update`, { assetIds, updates });
}

export async function exportAssets(filters?: AssetFilters, format: "csv" | "xlsx" = "csv") {
